# Filtering and State Eimstation
...
1. **[data_logger.py](code/local/data_logger.py)**
...
2. **[live_plotter.py](code/local/live_plotter.py)**
...
3. **[udp_client.py](code/local/udp_client.py)**
...
4. **[udp_client_example.py](code/local/udp_client_example.py)**
...
1. **Locating Q and R**
    ...
    - `Q` represents how uncertain you believe your _process_ model is (i.e., how accurate you think your encoder-based estimate is).
    - `R` represents how uncertain you believe your _sensor_ (IMU) measurement is.
...
2. **Synchronize Changes**
    ...
    - If you change `ARRAY_SIZE` on the server (Pico side), you must also change the client-side code that _receives_ the data. The data format uses `struct.pack` in chunks of `ARRAY_SIZE` floats, so the client needs to expect the same number.
